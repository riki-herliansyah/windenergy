...
const energyMean = (arr) => Math.pow(arr.reduce((s,v)=>s+v*v*v,0)/arr.length, 1/3);
    ...
    <Section num="0" title="Rata-rata Aritmetik vs Energy-Mean dalam Estimasi Daya Angin">
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:20 }}>
        ...
        <div style={{ padding:"16px", background:"#080e1c", borderRadius:12, border:`1px solid ${C.green}40`, borderTop:`3px solid ${C.green}` }}>
          ...
          <div style={{ textAlign:"center", marginBottom:10 }}>
            <Formula size={17}>v̄_EM = [ (1/n) · Σ v^3 ]^(1/3)</Formula>
          </div>
          <p style={{ fontSize:12, color:C.muted, margin:0, lineHeight:1.65 }}>
            Akar kubik dari rata-rata kubik kecepatan. Lebih tepat karena mempertimbangkan
            bobot <strong style={{color:C.text}}>v^3 dalam perhitungan daya</strong>. Meminimalkan
          ...
          <div key={item.label} style={{
            ...
            <div style={{ marginTop:8, fontSize:12, color: item.err===0?C.amber : item.err<-0.1?C.red:C.green }}>
              {item.err===0
                ? "✓ Referensi"
                : item.err<0
                  ? `▼ ${fmt(Math.abs(item.err),1)}% sama dengan aktual`
